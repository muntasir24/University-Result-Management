...
session_start();
error_reporting(0);
require('../admin/TCPDF-main/tcpdf.php');
include '../admin/includes/database.php';
include '../admin/includes/functions.php';

$conn = new Functions();
$redirect = $conn->student_url();
$back_to_terminal_report = $conn->base_url();

$exam = str_replace('-', ' ', $_POST['exam']);
$dept = $_POST['dept'];
$session = $_POST['session'];

$sql = "SELECT is_published from publish where dept_name=:d and session=:ss and semester=:s";
$conn->query($sql);
$conn->bind(":d", $dept);
$conn->bind(":ss", $session);
$conn->bind(":s", $exam);
$flag = $conn->fetchSingle();
$flag = $flag->is_published;
...
if ($flag) {
    ...
    //echo $rowcount;
        ...
        if ($rowcount == $totSubj) {
            $pdf->Cell(95, 8, 'Course Name', 1, 0, 'L', 1);
            $pdf->Cell(47, 8, 'Course Code', 1, 0, 'L', 1);
            $pdf->Cell(47, 8, 'Letter Grade', 1, 0, 'L', 1);
            ...
            foreach ($result as $report_details) {
                ...
                $pdf->Cell(95, 8, $subjectName, 1, 0, "L");
                $pdf->Cell(47, 8, $subject, 1, 0, "L");
                $pdf->Cell(47, 8, $letter_grade, 1, 0, "L");
...
<script>
   ...
   <meta http-equiv="refresh" content="4; <?php echo $redirect; ?>">
